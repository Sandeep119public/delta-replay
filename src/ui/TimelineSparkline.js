...
export class TimelineSparkline {
  constructor({
    ...
    this.height = height;
    this.palette = {
      line: '#3b82f6', cursor: '#f8fafc', long: '#22c55e', short: '#ef4444',
      win: '#22c55e', loss: '#ef4444', ...(palette || {}),
    };
  ...
  render() {
    ...
    try {
      ctx.beginPath();
      closes.forEach((p, i) => { if (i === 0) ctx.moveTo(x(i), y(p)); else ctx.lineTo(x(i), y(p)); });
      ctx.strokeStyle = this.palette.line; ctx.lineWidth = 1.5; ctx.stroke();
    } catch {}
    try {
      ...
      for (const t of this._trades()) {
        ...
        if (entryIdx >= 0 && Number.isFinite(t?.entryPrice)) {
          const ex = x(entryIdx), ey = Math.min(Math.max(y(t.entryPrice), 4), h - 4);
        ...
        if (exitIdx >= 0 && Number.isFinite(t?.exitPrice)) {
          const ex = x(exitIdx), ey = Math.min(Math.max(y(t.exitPrice), 4), h - 4);
          ctx.beginPath(); ctx.arc(ex, ey, 3, 0, Math.PI * 2); ctx.fillStyle = Number(t?.netPnL ?? t?.realizedPnL ?? 0) >= 0 ? this.palette.win : this.palette.loss; ctx.fill();
        }
      }
    } catch {}
    try {
      ...
      if (cursor >= 0 && cursor < n) {
        ctx.beginPath(); ctx.moveTo(x(cursor), 0); ctx.lineTo(x(cursor), h); ctx.strokeStyle = this.palette.cursor; ctx.lineWidth = 1; ctx.stroke();
      }
    } catch {}
    return true;
  }
