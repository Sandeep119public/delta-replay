...
export function createPaperUI({ engine, candleStore, appState, coordinatorRef }) {
  ...
  const controls = new ReplayControls({
    playBtn: el('btn-play'), pauseBtn: el('btn-pause'), stepBtn: el('btn-step'),
    resetBtn: el('btn-reset'), startReplayBtn: el('start-replay-btn'),
    speedSelect: el('speed-select'), statusEl: el('replay-status'), engine,
...
function createPaperTerminalViews(ctx) {
  const {
    el, appState, candleStore, engine, tradingEngine, commandController, coordinator,
    timeframeSelect, orderTypeSelect, limitPriceInput, stopPriceInput, slInput, tpInput,
  } = ctx;

  const sparkline = new TimelineSparkline({
    canvasEl: el('timeline-sparkline'), candleStore, engine, tradingEngine,
  ...
  const dateSelector = new ReplayDateSelector({
    appState, coordinator, candleStore, engine, commandController, timeframeSelect,
    onJump: (idx) => commandController.trySeek(idx),
  });
  const tradingPanel = new TradingPanel({
    tradingEngine,
    balanceEl: el('acct-balance'), equityEl: el('acct-equity'),
    realizedEl: el('acct-realized'), unrealizedEl: el('acct-unrealized'),
    feesEl: el('acct-fees'), posSymbolEl: el('pos-symbol'), posSideEl: el('pos-side'),
    posQtyEl: el('pos-qty'), posEntryEl: el('pos-entry'), posCurrentEl: el('pos-current'),
    posPnlEl: el('pos-pnl'), qtyInput: el('trade-qty'), buyBtn: el('btn-buy'),
    sellBtn: el('btn-sell'), closeBtn: el('btn-close'), resetBtn: el('btn-reset-acct'),
    tradesListEl: el('trades-list'), errorEl: el('trading-error'), orderTypeSelect,
    limitPriceInput, stopPriceInput, pendingListEl: el('pending-orders-list'),
    posSlEl: el('pos-sl'), posTpEl: el('pos-tp'), slInput, tpInput,
