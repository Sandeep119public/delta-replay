...
export function createTerminalUI({ mount, replayPort, commandPort, trading = null, tradingEvents = null, dataset = null, candles = null, chart = null, callbacks = {} }) {
  if (!mount || !replayPort || !commandPort || !dataset || !chart?.chartManager || !chart?.adapter) throw new TypeError('createTerminalUI requires mount, ports, dataset and chart handles');
  ...
  const controls = new ReplayControls({ playBtn: el('btn-play'), pauseBtn: el('btn-pause'), stepBtn: el('btn-step'), resetBtn: el('btn-reset'), startReplayBtn: null, speedSelect: el('speed-select'), statusEl: el('replay-status'), replayPort, commandPort, followBtn: el('btn-follow'), onFollowClick: onFollow });
