...
export function createReplayUI({ engine, candleStore, appState, coordinatorRef }) {
  ...
  return { el, chartManager, adapter, symbolSelector, timeframeSelector, timeline, controls, errorPanel, themeManager, modeBanner: new ModeBanner(), tradingErrorView: new TradingErrorView({ element: el('trading-error') }) };
}
