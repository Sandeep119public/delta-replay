...
export class RemoteTradingEngine {
  ...
  _emitStateTransitions(previous, action, backendEvents) {
    ...
    for (const order of afterOrders) {
      ...
      if (before?.status !== order.status && order.status === 'FILLED') { this.events.emit(EVENT.ORDER_TRIGGERED, clone(order)); this.events.emit(EVENT.ORDER_FILLED, { order: clone(order) }); }
      if (before?.status !== order.status && order.status === 'CANCELLED') this.events.emit(EVENT.ORDER_CANCELLED, clone(order));
      if (before?.status !== order.status && order.status === 'REJECTED') this.events.emit(EVENT.ORDER_REJECTED, clone(order));
    }
    ...
    for (const event of backendEvents) { const type = String(event?.type || '').toUpperCase(); if (type === 'ORDER_FILLED') this.events.emit(EVENT.ORDER_FILLED, clone(event)); else if (type === 'ORDER_REJECTED') this.events.emit(EVENT.ORDER_REJECTED, clone(event)); else if (type === 'STOP_LOSS') this.events.emit(EVENT.STOP_LOSS_TRIGGERED, clone(event)); else if (type === 'TAKE_PROFIT') this.events.emit(EVENT.TAKE_PROFIT_TRIGGERED, clone(event)); else if (type === 'LIQUIDATION') this.events.emit(EVENT.POSITION_LIQUIDATED, clone(event)); }
