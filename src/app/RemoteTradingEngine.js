...
class Events {
  ...
  emit(event, payload) {
    for (const handler of this.map.get(event) || []) {
      try { handler(payload); } catch (error) { console.warn(`[RemoteTradingEngine] ${event} handler failed`, error); }
    }
  }
}
...
function normalizePosition(raw) {
  ...
  const unrealizedPnL = number(raw.unrealizedPnL ?? raw.unrealized_pnl,
    entryPrice != null && currentPrice != null
      ? (currentPrice - entryPrice) * quantity * (side === 'SHORT' ? -1 : 1)
      : 0);
  return {
    ...raw,
    ...
    currentPrice,
    unrealizedPnL,
    stopLossPrice: number(raw.stopLossPrice ?? raw.stop_loss),
    takeProfitPrice: number(raw.takeProfitPrice ?? raw.take_profit),
  };
}
...
function normalizeAccount(raw = {}) {
  return {
    ...raw,
    startingBalance: number(raw.startingBalance ?? raw.starting_balance, 0),
    cashBalance: number(raw.cashBalance ?? raw.walletBalance ?? raw.wallet_balance, 0),
    walletBalance: number(raw.walletBalance ?? raw.wallet_balance ?? raw.cashBalance, 0),
...
export class RemoteTradingEngine {
  ...
  _sync(response = {}, action = null, previous = this.data) {
    ...
    if (Array.isArray(response.pendingOrders)) this.data.pendingOrders = response.pendingOrders;
    else this.data.pendingOrders = this.data.orders.filter((order) => order.status === 'PENDING');
  ...
  _emitStateTransitions(previous, action, backendEvents) {
    const beforePositions = previous.positions || [];
    const afterPositions = this.data.positions || [];
    const beforeOrders = previous.orders || [];
    const afterOrders = this.data.orders || [];
    const beforeTrades = previous.trades || [];
    ...
    if (afterPositions.length === beforePositions.length && afterPositions.length > 0) {
      const before = beforePositions[0];
      const after = afterPositions[0];
      if (JSON.stringify(before) !== JSON.stringify(after)) this.events.emit(EVENT.POSITION_UPDATED, after);
    }
    ...
    for (const order of afterOrders) {
      ...
      if (before?.status !== order.status && order.status === 'FILLED') {
        this.events.emit(EVENT.ORDER_TRIGGERED, order);
        this.events.emit(EVENT.ORDER_FILLED, { order });
      }
    ...
    if (this.data.trades.length > beforeTrades.length) {
      this.events.emit(EVENT.TRADE_EXECUTED, this.data.trades[this.data.trades.length - 1]);
    }

    for (const event of backendEvents) {
      const type = String(event?.type || '').toUpperCase();
      if (type === 'ORDER_FILLED') this.events.emit(EVENT.ORDER_FILLED, event);
      else if (type === 'ORDER_REJECTED') this.events.emit(EVENT.ORDER_REJECTED, event);
      else if (type === 'STOP_LOSS') this.events.emit(EVENT.STOP_LOSS_TRIGGERED, event);
      else if (type === 'TAKE_PROFIT') this.events.emit(EVENT.TAKE_PROFIT_TRIGGERED, event);
      else if (type === 'LIQUIDATION') this.events.emit(EVENT.POSITION_LIQUIDATED, event);
    }
  ...
  getPerformanceStats() {
    ...
    const grossWin = wins.reduce((sum, trade) => sum + Number(trade.netPnL || 0), 0);
    const grossLoss = Math.abs(trades.filter((trade) => Number(trade.netPnL ?? 0) < 0).reduce((sum, trade) => sum + Number(trade.netPnL || 0), 0));
    const net = trades.reduce((sum, trade) => sum + Number(trade.netPnL || 0), 0);
    const starting = Number(this.data.account.startingBalance || 0);
    return {
      totalTrades: trades.length,
      winRate: trades.length ? (wins.length / trades.length) * 100 : 0,
      profitFactor: grossLoss ? grossWin / grossLoss : (grossWin ? Infinity : 0),
      netReturn: starting > 0 ? (net / starting) * 100 : 0,
    };
  }
  ...
  async _action(path, options, action) {
    try {
      const response = await this._request(path, options, action);
      return { success: true, ...response };
    } catch (error) {
      return { success: false, message: error?.message || 'Trading request failed', status: error?.status, error };
    }
  }
  ...
  submitMarketOrder(order) {
    return this._action('/order', {
      method: 'POST',
      body: JSON.stringify({ symbol: order.symbol, side: String(order.side).toLowerCase(), quantity: order.quantity, type: 'market' }),
    }, 'place');
  }
  ...
  placeLimitOrder(order) {
    return this._action('/order', {
      method: 'POST',
      body: JSON.stringify({ symbol: order.symbol, side: String(order.side).toLowerCase(), quantity: order.quantity, type: 'limit', limitPrice: order.limitPrice }),
    }, 'place');
  }
  ...
  placeStopOrder(order) {
    return this._action('/order', {
      method: 'POST',
      body: JSON.stringify({ symbol: order.symbol, side: String(order.side).toLowerCase(), quantity: order.quantity, type: 'stop_market', stopPrice: order.stopPrice }),
    }, 'place');
  }
  ...
  updateRisk({ symbol, stopLoss, takeProfit }) {
    return this._action('/risk', { method: 'POST', body: JSON.stringify({ symbol: String(symbol).toUpperCase(), stopLoss, takeProfit }) }, 'risk');
  }

  setRisk({ symbol, stopLoss, takeProfit }) { return this.updateRisk({ symbol, stopLoss, takeProfit }); }

  setStopLoss(symbol, price) {
    const position = this.getPositions().find((candidate) => candidate.symbol === String(symbol).toUpperCase());
    return this.updateRisk({ symbol, stopLoss: price, takeProfit: position?.takeProfitPrice ?? null });
  }

  setTakeProfit(symbol, price) {
    const position = this.getPositions().find((candidate) => candidate.symbol === String(symbol).toUpperCase());
    return this.updateRisk({ symbol, stopLoss: position?.stopLossPrice ?? null, takeProfit: price });
  }
  ...
  clearStopLoss(symbol) { return this.clearRisk(symbol); }
  clearTakeProfit(symbol) { return this.clearRisk(symbol); }
  ...
  async onMarketCandle(payload = null) {
    ...
    if (candle) this.latestCandle = candle;
    const body = candle
      ? JSON.stringify({ symbol: String(payload.symbol || candle.symbol || 'BTCUSDT').toUpperCase(), candle, index: payload.index ?? null })
      : undefined;
    return this._request('/candle', { method: 'POST', ...(body ? { body } : {}) }, 'candle');
  }
