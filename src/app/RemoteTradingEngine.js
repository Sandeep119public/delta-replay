...
export class RemoteTradingEngine {
  ...
  submitOrder(order) {
    ...
    const payload = {
      ...
      type: type === 'stop_market' ? 'stop_market' : type === 'limit' ? 'limit' : 'market',
      ...(type === 'limit' ? { limitPrice: order.limitPrice } : {}),
      ...(type === 'stop_market' ? { stopPrice: order.stopPrice } : {}),
    };
    return this._action('/order', { method: 'POST', body: JSON.stringify(payload) }, 'place');
  }
  ...
  clearRisk(symbol, target = 'all') {
    const normalized = String(symbol).toUpperCase();
    const query = target === 'all' ? '' : `&target=${encodeURIComponent(target)}`;
    return this._action(`/risk/clear?symbol=${encodeURIComponent(normalized)}${query}`, { method: 'POST' }, 'risk');
  }
  ...
  reset() { return this._action('/reset', { method: 'POST' }, 'reset'); }
