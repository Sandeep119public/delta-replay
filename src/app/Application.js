...
export function createApplication() {
  ...
  const ui = createPaperUI({ engine, replayPort, candleStore, appState, coordinatorRef, tradingPort, tradingEvents, tradingState });
  const coordinator = new ReplayCoordinator({ dataManager, candleStore, appState, replayEngine: engine, tradingEngine, chartManager: ui.chartManager, chartAdapter: ui.adapter, timeline: ui.timeline, controls: ui.controls, errorPanel: ui.errorPanel, modeBanner: ui.modeBanner, tradingErrorView: ui.tradingErrorView, ...ui.getCoordinatorPorts() });
  coordinatorRef.current = coordinator;
  const coordinatorPorts = ui.getCoordinatorPorts();
  const commandController = new ReplayCommandController({ engine, appState, candleStore, tradingEngine, coordinator, headerBtn: coordinatorPorts.headerStartReplayBtn, onError: (msg) => coordinator.showTradingError(msg) });
  ...
  const views = ui.createTerminalViews({ appState, candleStore, engine, replayPort, tradingPort, tradingEvents, tradingState, commandController, coordinator, timeline: ui.timeline, controls: ui.controls, modeBanner: ui.modeBanner, ...form });
  ...
  const loadBtn = coordinatorPorts.loadBtn; const onLoadClick = () => actions.load(); loadBtn?.addEventListener('click', onLoadClick);
  const loadBinding = { destroy() { loadBtn?.removeEventListener?.('click', onLoadClick); } };
  const mobileDrawer = bindMobileDrawer();
  const destroy = bindApplicationLifecycle({ unbindKeyboardShortcuts, coordinator, engine, candleCache, resources: [selectorBindings, timelineBindings, tradingBindings, tradingStateBridge, replayLifecycle, commandController, mobileDrawer, loadBinding, ui.symbolSelector, ui.timeframeSelector, ui.timeline, ui.controls, ui.themeManager, ui.errorPanel, chartTradingController, ui.adapter, ui.chartManager, views.tradingPanel, views.dateSelector, views.sparkline, views.floatingPosView, views.toastView], extraCleanup: [unbindAutoFollow, actionGuardUnsub] });
  let started = false, destroyed = false;
  const guardedDestroy = () => { if (destroyed) return; destroyed = true; destroy(); };
  return { start() { if (destroyed || started) return; started = true; ui.modeBanner.update({ replayState: engine.getState(), appState, candleStore }); Promise.resolve(coordinator.loadAndPrepareReplay({ autoStart: false })).catch((error) => { if (!destroyed) coordinator.showTradingError?.(error?.message || 'Failed to load replay'); }); }, destroy: guardedDestroy, services, ui, coordinator };
}
