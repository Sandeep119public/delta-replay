...
export function createApplication() {
  ...
  const coordinator = new ReplayCoordinator({ dataManager, candleStore, appState, replayEngine: engine, tradingEngine, chartManager: ui.chartManager, chartAdapter: ui.adapter, timeline: ui.timeline, controls: ui.controls, errorPanel: ui.errorPanel, modeBanner: ui.modeBanner, tradingErrorView: ui.tradingErrorView, dataStatusEl: ui.el('data-status'), cacheBadgeEl: ui.el('cache-badge'), startReplayBtn: ui.el('start-replay-btn'), headerStartReplayBtn: ui.el('header-start-replay-btn'), loadBtn: ui.el('load-btn'), fromDateEl: ui.el('from-date'), fromTimeEl: ui.el('from-time'), toDateEl: ui.el('to-date'), toTimeEl: ui.el('to-time') });
  coordinatorRef.current = coordinator;
  engine.registerActionGuard((action) => { if (!tradingEngine.hasOpenPosition()) return { allowed: true }; const msg = action === 'load' ? 'Cannot load new data while a position is open — close position or reset account first.' : `Cannot ${action} while a position is open — close position first.`; coordinator.showTradingError(msg); return { allowed: false, reason: msg }; });
  const commandController = new ReplayCommandController({ engine, appState, candleStore, tradingEngine, coordinator, headerBtn: ui.el('header-start-replay-btn'), onError: (msg) => coordinator.showTradingError(msg) });
  const unbindKeyboardShortcuts = commandController.bindKeyboardShortcuts();
  const form = { timeframeSelect: ui.el('timeframe-select'), orderTypeSelect: ui.el('order-type'), limitPriceInput: ui.el('limit-price'), stopPriceInput: ui.el('stop-price'), slInput: ui.el('sl-price'), tpInput: ui.el('tp-price') };
  ...
  bindTimelineInteractions({ timeline: ui.timeline, controls: ui.controls, appState, engine, candleStore, tradingEngine, commandController, coordinator, modeBanner: ui.modeBanner });
  ...
  new ChartTradingController({ chartManager: ui.chartManager, tradingEngine, tradingPanel: views.tradingPanel, floatingPosView: views.floatingPosView, toastView: views.toastView, orderFormView: views.tradingPanel.orderFormView, coordinator, ...form });
  ...
  bindMobileDrawer();
  const destroy = bindApplicationLifecycle({ unbindKeyboardShortcuts, coordinator, engine, candleCache });
