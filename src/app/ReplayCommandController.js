...
export class ReplayCommandController {
  ...
  hasData() { return !this.destroyed && (this.candleStore?.getCount?.() || 0) > 0; }
  allowed(action) {
    if (this.destroyed || this.busy) return false;
    try { const result = this.canExecute(action); if (result?.allowed === false) { this.error(result.reason); return false; } return result !== false; }
  ...
  async stepForward() { if (this.destroyed || this.busy) return false; return this.hasData() && this.allowed('stepForward') ? this.run(() => this.engine.stepForward()) : false; }
  async stepBackward() { if (this.destroyed || this.busy) return false; const n = this.engine.getState().currentIndex - 1; return n >= 0 ? this.trySeek(n) : false; }
  async jumpBy(delta) {
    if (this.destroyed || this.busy) return false;
    const amount = Number(delta);
    if (!Number.isFinite(amount) || !Number.isInteger(amount)) return false;
    const s = this.engine.getState(); const total = this.engine.getTotalCandles();
