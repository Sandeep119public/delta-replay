...
export class RemoteReplayEngine {
  ...
  async _call(path, options = {}) { const response = await this.api.request(path, options); this._sync(response); return this.getState(); }
  ...
  async load(candles) { this.pause(); return this._call('/load', { method: 'POST', body: JSON.stringify({ candles: Array.isArray(candles) ? candles : [] }) }); }
  async start(index = 0) { const result = await this._call(`/start/${Number(index)}`, { method: 'POST' }); this.events.emit('started', { index: this.state.currentIndex, state: result }); return result; }
  async stepForward() {
  ...
  async seek(index) { this.pause(); const result = await this._call(`/seek/${Number(index)}`, { method: 'POST' }); this.events.emit('seeked', { index: this.state.currentIndex, state: result }); return result; }
  async reset() { this.pause(); const result = await this._call('/reset', { method: 'POST' }); this.events.emit('reset', { index: this.state.currentIndex, state: result }); return result; }
  async play() { if (this.state.status === 'ready') await this.start(this.state.startIndex >= 0 ? this.state.startIndex : 0); if (!['paused', 'playing'].includes(this.state.status)) return this.getState(); if (this._playTimer) return this.getState(); this.state = { ...this.state, status: 'playing' }; this.events.emit('stateChanged', this.getState()); this._scheduleStep(); return this.getState(); }
  pause() { if (this._playTimer) clearTimeout(this._playTimer); this._playTimer = null; if (this.state.status === 'playing') { this.state = { ...this.state, status: 'paused' }; this.events.emit('stateChanged', this.getState()); } return this.getState(); }
  _scheduleStep() { if (this.state.status !== 'playing' || this._playTimer) return; const delay = Math.max(40, BASE_STEP_DELAY_MS / Number(this.state.speed || 1)); this._playTimer = setTimeout(async () => { this._playTimer = null; if (this.state.status !== 'playing') return; try { await this.stepForward(); } catch (error) { this.pause(); this.events.emit('playbackError', error); return; } if (this.state.status === 'playing') this._scheduleStep(); }, delay); }
  setSpeed(speed) { const next = Number(speed); if (!Number.isFinite(next) || next <= 0) throw new TypeError('Replay speed must be a positive number'); this.state = { ...this.state, speed: next }; this.events.emit('speedChanged', { speed: next }); this.events.emit('stateChanged', this.getState()); if (this.state.status === 'playing') { if (this._playTimer) clearTimeout(this._playTimer); this._playTimer = null; this._scheduleStep(); } return next; }
  registerActionGuard() { return () => {}; }
  destroy() { this.pause(); this.events = new Events(); this.tradingEngine = null; }
}
