...
export class RemoteReplayEngine{
 ...
 async _call(path,opts){const data=await this.api.request(path,opts);this._sync(data);if(this.tradingEngine&&data.candle&&path==='/step')await this.tradingEngine.onMarketCandle();return this.state}
 ...
 async start(i=0){return this._call('/start/'+i,{method:'POST'})}async stepForward(){return this._call('/step',{method:'POST'})}async seek(i){return this._call('/seek/'+i,{method:'POST'})}async reset(){return this._call('/reset',{method:'POST'})}
