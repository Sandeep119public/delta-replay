...
export class RemoteReplayEngine {
  ...
  async play() {
    ...
    if (this.state.status === 'ready') {
      const expectedGeneration = this._generation + 1;
      await this.start(this.state.startIndex >= 0 ? this.state.startIndex : 0);
      if (this._destroyed || this._generation !== expectedGeneration) return this.getState();
    }
    if (this._destroyed || !['paused', 'playing'].includes(this.state.status)) return this.getState();
  ...
  destroy() { if (this._destroyed) return; if (this._playTimer) clearTimeout(this._playTimer); this._playTimer = null; this._destroyed = true; this._generation++; this.events = new Events(); this.tradingEngine = null; }
}
