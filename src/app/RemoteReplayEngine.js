...
export class RemoteReplayEngine {
 constructor(api){this.api=api;this.events=new Events();this.state={status:'idle',currentIndex:-1,startIndex:-1,total:0,speed:1}}
 async _call(path,opts){const data=await this.api.request(path,opts);this._sync(data);return this.state}
 ...
 async load(candles){return this._call('/load',{method:'POST',body:JSON.stringify({candles})})}
