...
export class RemoteReplayEngine {
  constructor(api, tradingEngine = null, symbolProvider = () => 'BTCUSDT') {
    ...
    if (typeof symbolProvider !== 'function') throw new TypeError('RemoteReplayEngine symbolProvider must be a function');
    this.api = api;
  ...
  async _drainMutations() {
    ...
    try {
      while (this._mutationQueue.length) {
        ...
        if (this._destroyed) { item.resolve(this.getState()); continue; }
        try { item.resolve(await item.operation()); }
        catch (error) { item.reject(error); }
      }
    } finally {
      this._mutationRunning = false;
      if (this._mutationQueue.length && !this._destroyed) void this._drainMutations();
    }
  }
  ...
  load(candles) { if (this._destroyed) return Promise.resolve(this.getState()); this.pause(); const generation = ++this._generation; return this._enqueueMutation(() => this._call('/load', { method: 'POST', body: JSON.stringify({ candles: Array.isArray(candles) ? clone(candles) : [] }) }, generation, 'load')); }
  start(index = 0, symbol = null) { if (this._destroyed) return Promise.resolve(this.getState()); const numericIndex = Number(index); if (!Number.isInteger(numericIndex)) return Promise.reject(new TypeError('Replay start index must be an integer')); const normalizedSymbol = String(symbol ?? this.currentSymbol()).trim().toUpperCase(); if (!normalizedSymbol) return Promise.reject(new TypeError('Replay symbol must be provided')); const generation = ++this._generation; return this._enqueueMutation(async () => { const result = await this._call(`/start/${numericIndex}?symbol=${encodeURIComponent(normalizedSymbol)}`, { method: 'POST' }, generation, 'start'); if (!this._destroyed && generation === this._generation) this.events.emit('started', { index: this.state.currentIndex, state: result }); return result; }); }
  stepForward(symbol = null) {
    if (this._destroyed || this._stepInFlight) return Promise.resolve(this.getState());
    if (this.state.currentIndex < 0 || this.state.currentIndex >= this.state.totalCandles - 1) { if (this.state.totalCandles > 0 && this.state.currentIndex >= this.state.totalCandles - 1) this.pause(); return Promise.resolve(this.getState()); }
  ...
  seek(index) { if (this._destroyed) return Promise.resolve(this.getState()); this.pause(); const numericIndex = Number(index); if (!Number.isInteger(numericIndex)) return Promise.reject(new TypeError('Replay seek index must be an integer')); const generation = ++this._generation; return this._enqueueMutation(async () => { const result = await this._call(`/seek/${numericIndex}?symbol=${encodeURIComponent(this.currentSymbol())}`, { method: 'POST' }, generation, 'seek'); if (!this._destroyed && generation === this._generation) this.events.emit('seeked', { index: this.state.currentIndex, state: result }); return result; }); }
  reset() { if (this._destroyed) return Promise.resolve(this.getState()); this.pause(); const generation = ++this._generation; return this._enqueueMutation(async () => { const result = await this._call('/reset', { method: 'POST' }, generation, 'reset'); if (!this._destroyed && generation === this._generation) this.events.emit('reset', { index: this.state.currentIndex, state: result }); return result; }); }
  async play() {
    if (this._destroyed) return this.getState();
    const playIntent = ++this._playIntent;
    if (this.state.status === 'ready') await this.start(this.state.startIndex >= 0 ? this.state.startIndex : 0, this.currentSymbol());
  ...
  _scheduleStep() { if (this._destroyed || this.state.status !== 'playing' || this._playTimer) return; const delay = Math.max(40, BASE_STEP_DELAY_MS / Number(this.state.speed || 1)); this._playTimer = setTimeout(async () => { this._playTimer = null; if (this._destroyed || this.state.status !== 'playing') return; try { await this.stepForward(this.currentSymbol()); } catch (error) { this.pause(); if (!this._destroyed) this.events.emit('playbackError', error); return; } if (!this._destroyed && this.state.status === 'playing') this._scheduleStep(); }, delay); }
  ...
  destroy() { if (this._destroyed) return; this.pause(); this._destroyed = true; this._generation++; for (const item of this._mutationQueue.splice(0)) item.resolve(this.getState()); this.events = new Events(); this.tradingEngine = null; }
}
