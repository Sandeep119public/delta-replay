...
export class PaperTradingEngine extends EventEmitter {
  ...
  attachToReplay(replayEngine) {
    ...
    const clearOnLifecycle = () => {
      // A replay load/reset starts a new deterministic simulation session.
  ...
  onMarketCandle(payload) {
    ...
    // ReplayEngine.start() emits the selected candle, while the UI may also seed that same candle in preview mode.
    // Do not evaluate a logical candle more than once.
    if (previousMarket && previousMarket.index === idx && previousMarket.timestamp === c.time) return;
    ...
    this._isProcessingCandle = false;
    this.emit(TradingEvents.ACCOUNT_UPDATED, this.getAccountSnapshot());
    this.emit(TradingEvents.BAR_CLOSE, { index: idx, timestamp: c.time, candle: Object.freeze({ ...this._latestCandle }), phase: 'BAR_CLOSE' });
  }
