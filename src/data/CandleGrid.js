...
export function normalizeRange(from, to, tfSec, origin = 0) {
  ...
  return { requestedFrom: from, requestedTo: to, effectiveFrom, effectiveTo, hasCandle };
}

export function getExpectedGridTimestamps(from, to, tfSec) {
  if (!Number.isFinite(from) || !Number.isFinite(to) || !Number.isFinite(tfSec) || tfSec <= 0 || from > to) return [];
  const timestamps = [];
  for (let t = from; t <= to; t += tfSec) timestamps.push(t);
  return timestamps;
}
...
export function mergeGridIntervals(intervals, tfSec = 1) {
  ...
  for (let i = 1; i < clean.length; i++) {
    ...
    if (cur.from <= last.to + step) last.to = Math.max(last.to, cur.to);
...
export function computeGridMissing(requestedFrom, requestedTo, cachedIntervals = [], tfSec = 60) {
  ...
  if (!cachedIntervals || cachedIntervals.length === 0) return [{ from: start, to: end }];
  ...
  for (const iv of sorted) {
    if (iv.from > cursor) missing.push({ from: cursor, to: Math.min(end, iv.from - step) });
...
export function intervalsFromCandles(candles, tfSec) {
  ...
  const pushInterval = (s, p, cnt) => {
    ...
    Object.defineProperty(item, 'count', { value: cnt, enumerable: false, writable: true, configurable: true });
    intervals.push(item);
  };
