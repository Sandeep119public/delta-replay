...
export function downloads(snapshot, download, dates) {
  ...
  return `<div class="data-page">${header('DATA / DOWNLOADS','Download Center','Download validated historical Binance candles and publish them as an immutable replay dataset to GitHub.')}${error}<section class="data-panel"><form class="data-form" novalidate><label>Symbol<input id="data-symbol" value="${escapeText(symbol)}" autocomplete="off" spellcheck="false"></label><label>Timeframe<select id="data-timeframe">${timeframes.map((tf)=>`<option value="${tf}" ${tf===timeframe?'selected':''}>${tf}</option>`).join('')}</select></label><label>Start<input id="data-from" type="datetime-local" value="${dates.start}"></label><label>End<input id="data-to" type="datetime-local" value="${dates.end}"></label><button class="data-primary" type="button" data-data-action="download" ${['running','starting'].includes(download.status)?'disabled':''} >${download.status==='complete'?'Download and publish again':'Download and publish'}</button></form>${progress}<p class="data-note">Downloads use Binance historical klines, cache reuse, retry and integrity checks. A completed download is published as an immutable GitHub replay dataset.</p></section><section class="data-panel"><h2>Download lifecycle</h2><ol class="data-steps"><li>Normalize the requested candle range.</li><li>Reuse clean cached coverage where possible.</li><li>Fetch missing ranges with retry and integrity checks.</li><li>Publish the validated candles as an immutable GitHub dataset.</li></ol></section></div>`;
}
