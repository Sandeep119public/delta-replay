...
export function downloads(snapshot, download, dates) {
  ...
  return `<div class="data-page">${header('DATA / DOWNLOADS','Download Center','Download validated historical Binance candles and publish them as an immutable replay dataset to GitHub.')}${error}<section class="data-panel"><form class="data-form" novalidate><label>Symbol<input id="data-symbol" value="${escapeText(symbol)}" autocomplete="off" spellcheck="false"></label><label>Timeframe<select id="data-timeframe">${timeframes.map((tf)=>`<option value="${tf}" ${tf===timeframe?'selected':''}>${tf}</option>`).join('')}</select></label><label>Start<input id="data-from" type="datetime-local" value="${dates.start}"></label><label>End<input id="data-to" type="datetime-local" value="${dates.end}"></label><button class="data-primary" type="button" data-data-action="download" ${['running','starting'].includes(download.status)?'disabled':''} >${download.status==='complete'?'Download and publish again':'Download and publish'}</button>${['running','starting'].includes(download.status)?'<button type="button" data-data-action="cancel-download">Cancel</button>':''}</form>${progress}<p class="data-note">Downloads are server-owned Binance jobs. Render fetches, validates, partitions and atomically publishes the immutable GitHub dataset.</p></section><section class="data-panel"><h2>Download lifecycle</h2><ol class="data-steps"><li>Normalize the requested candle range.</li><li>Run the download on the server.</li><li>Fetch Binance Futures klines with pagination and integrity checks.</li><li>Publish the validated candles as an immutable GitHub dataset.</li></ol></section></div>`;
}
  ...
  const rows = savedDatasets.length
    ? savedDatasets.map((dataset) => `
      <tr>
        ...
        <td>${(Number(dataset.partitions?.reduce?.((sum, part) => sum + Number(part.byteLength || 0), 0) || dataset.byteLength || 0) / 1048576).toFixed(2)} MB</td>
