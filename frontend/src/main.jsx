...
const initialAccount = { startingBalance: 10000, walletBalance: 10000, equity: 10000, unrealizedPnL: 0, totalFees: 0, position: null };
...
function App() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      request('/trading/state'),
    ]).then(([replay, trading]) => {
      setState(replay);
      setAccount(readAccount(trading));
      setConnected(true);
    }).catch((error) => {
      setConnected(false);
      setTradeError(errorMessage(error));
  ...
  const applyCandles = async (candles) => {
    ...
    setPlaying(false);
    setConnected(true);
    await request('/trading/reset', { method: 'POST' });
    await syncAccount();
  };
  ...
  const step = async () => {
    try {
      const data = await request(`/replay/step?symbol=${encodeURIComponent(SYMBOL)}`, { method: 'POST' });
      setState(data);
      setPlaying(false);
      setAccount(readAccount(data.trading));
    } catch (error) {
      setTradeError(errorMessage(error));
    }
  };
  ...
  const reset = async () => {
    try {
      const data = await request('/replay/reset', { method: 'POST' });
      setState(data);
      setAccount(readAccount(data.trading));
      setPlaying(false);
    } catch (error) {
  ...
  const seek = async (index) => {
    try {
      const data = await request(`/replay/seek/${index}`, { method: 'POST' });
      setState(data);
      setPlaying(false);
      setAccount(readAccount(data.trading));
    } catch (error) {
  ...
  const togglePlay = () => {
    if (state.index >= 0 && state.index < state.total - 1) setPlaying((value) => !value);
  };

  const trade = async (side) => {
    setTradeError('');
    try {
      const data = await request('/trading/order', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ symbol: SYMBOL, side, quantity: Number(quantity) }),
      });
      setAccount(readAccount(data));
    } catch (error) {
      setTradeError(errorMessage(error));
  ...
  const closePosition = async () => {
    setTradeError('');
    try {
      const data = await request('/trading/close', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
