...
export const LAYERS = [
  'core', 'data', 'indicators', 'state', 'app', 'chart', 'ui', 'pages', 'router', 'utils', 'ports', 'personality',
];
...
export const ALLOWED = {
  ...
  app: new Set(['core', 'data', 'indicators', 'state', 'chart', 'ui', 'pages', 'router', 'utils', 'ports', 'personality']),
  chart: new Set(['utils', 'ports']),
...
export const OWNERSHIP = [
  ['src/app/', 'application composition, remote adapters, runtime wiring, lifecycle, intent bridges'],
  ...
  ['src/state/', 'application/session state only; never historical candle storage'],
  ['src/core/', 'framework-neutral core primitives'],
  ['src/indicators/', 'indicator calculations and indicator data'],
  ['src/pages/', 'page-level composition'],
  ['src/router/', 'route selection and navigation wiring'],
...
export const RULES = [
  ...
  'CandleStore is the sole owner of the loaded replay candle dataset; AppState must not own or expose candle collections.',
  'ReplayEngine is the sole owner of replay cursor/lifecycle state; UI must use ReplayUIPort for replay data and commands.',
  'ReplayLoadService is the single workflow for reading and validating a selected replay dataset.',
  'Timeline stores timestamps and markers, not candle objects.',
  'Chart preview and playback must use the same ReplayUIPort data path.',
  'Replay candle consumers must be serialized with cursor advancement; a failed consumer must not silently advance replay state.',
...
export const IMPACT_ORDER = ['Architecture', 'UI', 'UI contracts', 'Data/state', 'Tests', 'Tooling', 'Guidance', 'Backend', 'CI'];
