...
describe('remote contracts', () => {
  ...
  it('rejects fractional replay indices before making requests', async () => {
    ...
    const engine = new RemoteReplayEngine(client);
    await engine.load([candle]);
    await expect(engine.start(0.5)).rejects.toThrow(/integer/);
    await expect(engine.seek(0.5)).rejects.toThrow(/integer/);
  ...
  it('serializes trading mutations in invocation order', async () => {
    let resolveFirst;
    const firstResponse = new Promise((resolve) => { resolveFirst = resolve; });
    let requestCount = 0;
    const client = {
      request: vi.fn((path) => {
        ...
        return requestCount === 1
          ? firstResponse
          : Promise.resolve({ account: { startingBalance: 12000, equity: 12000 }, positions: [], orders: [], trades: [] });
      }),
    };
    const engine = new RemoteTradingEngine(client);
  ...
  it('maps trading actions to canonical API payloads', async () => {
    ...
    const engine = new RemoteTradingEngine(client);
    const result = await engine.submitOrder({ symbol: 'BTCUSDT', side: 'BUY', quantity: 1, type: 'market' });
