...
describe('deep-audit refactor regressions', () => {
  it('cache does not claim a sparse timeframe range is fully covered', () => {
    const cache = new CandleCache({ enableIDB: false });
    cache.set('BTCUSDT', '1m', 1000, 1120, [candle(1000), candle(1120)]);

    const result = cache.get('BTCUSDT', '1m', 1000, 1120);

    expect(result.hit).toBe(false);
    expect(result.missing).toEqual([{ from: 1001, to: 1119 }]);
  });

  it('cache recognizes a contiguous timeframe range as covered', () => {
    const cache = new CandleCache({ enableIDB: false });
    cache.set('BTCUSDT', '1m', 1000, 1120, [candle(1000), candle(1060), candle(1120)]);
