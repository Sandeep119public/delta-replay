...
def test_backtest_endpoint_returns_net_trade_summary():
    candles = [
        {'time': 1, 'open': 100, 'high': 102, 'low': 99, 'close': 100, 'volume': 10},
    ...
    assert response.status_code == 200
    assert response.json()['summary']['pnl'] == pytest.approx(19.79)
