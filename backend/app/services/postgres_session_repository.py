...
class PostgresSessionRepository(SessionRepository):
    ...
    def _prepare_storage_document(cls, connection, document: SessionDocument) -> SessionDocument:
        ...
        if candles is not None:
            ...
            # Dataset reclamation and session writes must share the same
            # transaction-level lock. Acquire it before checking existence so a
            # concurrent GC cannot delete a dataset that this transaction will
            # later reference.
